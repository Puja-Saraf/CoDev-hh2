...
export default function MatchesDisplay({setClickedUser}) {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
          ...
          const data = await axios.get("http://localhost:8000/users/matched", {params});
          // console.log('here');
          // console.log(data);
        ...
        {users.map((user,_index) => (
            ...
            <div className='flex flex-row items-center justify-between h-16 w-[340px] md:w-[420px]'>
              <div className='h-[100%] flex flex-row items-center justify-center ml-4'>
                <img src={user.img_url ? user.img_url : maleUser} className='h-[70%] rounded-full cursor-pointer' onClick={()=>{navigate(`/profile/${user.user_id}`)}} />
                <div className='ml-2'>
                  <h2 className='text-base'>{user.name}</h2>
                  <h4 className=' text-xs opacity-70'>{user.professional_title}</h4>
                </div>
              </div>
              <i className="fa-regular fa-comment text-2xl mr-6 opacity-20 cursor-pointer" onClick={()=>{setClickedUser(user)}}></i>
            </div>
