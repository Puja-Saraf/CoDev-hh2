...
export default function RejectedUsers({setCurUser}) {
    ...
    useEffect(() => {
      ...
      const fetchData = async () => {
        ...
        const data = await axios.get("http://localhost:8000/users/reject", { params });
        // console.log('here');
        // console.log(data);
    ...
    return (
      <div className='flex flex-col justify-center mt-28 items-center'>
          <h1 className='text-2xl mb-4 text-[#fd2f6e] font-semibold'>Rejected Users</h1>
          <div className=' h-[1px] bg-slate-700 mt-2 mb-2 opacity-20 w-[340px] md:w-[420px]'></div>
         {users.map((user,_index)=>(<List user={user} key={_index} pending={false} setCurUser={setCurUser}/>))}
      </div>
    )
}
