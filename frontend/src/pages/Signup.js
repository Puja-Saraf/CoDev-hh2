...
export default function Signup() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (password === confPassword) {
      try {
        setLoading(true)
        data = await axios.post('http://localhost:8000/auth/signup', { email: email, password: password });
      ...
      <div className='flex flex-col lg:flex-row items-center justify-center bg-[#FFD9C0] bg-opacity-25 h-[100vh] bg-auto'>
        ...
        <div className='rounded-2xl bg-white drop-shadow-2xl overflow-auto mt-6'>
          <form onSubmit={handleSubmit} className='max-w-[320px] w-[240px] md:w-[300px] lg:w-[320px] mr-5 ml-5 mt-8 mb-8 md:mt-10 md:mb-10 md:mr-7 md:ml-7 lg:mr-10 lg:ml-10'>
            <h2 className='text-2xl md:text-3xl font-bold text-[#fd2f6e] text-center'>Signup</h2>
            <label className='block mr-auto ml-auto mt-6 mb-6 md:mt-8 md:mb-8'>
              <span className='block mb-2 text-left text-md md:text-lg text-[#2f2e41] font-medium'>Email:</span>
              <input className='p-2 md:p-3 w-[100%] border-[#2f2e41] border-2 rounded-lg focus:bg-[#FFD9C0] focus:bg-opacity-10' type='email' onChange={(e) => setEmail(e.target.value)} value={email} placeholder="Email" />
            </label>
            <label className='block mr-auto ml-auto mt-6 mb-6 md:mt-8 md:mb-8'>
              <span className='block mb-2 text-left text-md md:text-lg text-[#2f2e41] font-medium'>Set Password:</span>
              <input className='p-2 md:p-3 w-[100%] border-[#2f2e41] border-2 rounded-lg focus:bg-[#FFD9C0] focus:bg-opacity-10' type='password' onChange={(e) => setPassword(e.target.value)} value={password} placeholder="Password" maxLength={13} minLength={6}/>
            </label>
            <label className='block mr-auto ml-auto mt-6 mb-6 md:mt-8 md:mb-8'>
              <span className='block mb-2 text-left text-md md:text-lg text-[#2f2e41] font-medium'>Confirm Password:</span>
              <input className='p-2 md:p-3 w-[100%] border-[#2f2e41] border-2 rounded-lg focus:bg-[#FFD9C0] focus:bg-opacity-10' type='password' onChange={(e) => setConfPassword(e.target.value)} value={confPassword} placeholder="Password" maxLength={13} minLength={6}/>
            </label>
